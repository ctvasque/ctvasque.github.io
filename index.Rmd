...
cum_recovered      <- cum_recovered %>%
				dplyr::rename(recovered_cum = "Casos recuperados", date = Fecha) %>%
...
df_daily		 <- cum_confirmed %>%
				...
				tidyr::replace_na(list(recovered_cum = 0, death_cum = 0)) %>%
				dplyr::mutate(active_cum =  confirmed_cum - recovered_cum - death_cum) %>% 
...
casos_act	 <- casos_act 												%>%
				dplyr::rename(date = Fecha, Activos  = "Casos activos") 	%>%
...
shp_santiago = sf::st_as_sf(dplyr::filter(Nmapa_comunas,codigo_region=="13"))

assign(paste0("shp_",filtro_region$Region[1]),sf::st_as_sf(dplyr::filter(Nmapa_comunas,!("Código Comuna" %in% c("05104","05201")),codigo_region==filtro_region$codigo_region[1])) %>% dplyr::rename("Código Región" = codigo_region, "Región" = Region))
assign(paste0("shp_",filtro_region$Region[2]),sf::st_as_sf(dplyr::filter(Nmapa_comunas,!("Código Comuna" %in% c("05104","05201")),codigo_region==filtro_region$codigo_region[2])) %>% dplyr::rename("Código Región" = codigo_region, "Región" = Region))
assign(paste0("shp_",filtro_region$Region[3]),sf::st_as_sf(dplyr::filter(Nmapa_comunas,!("Código Comuna" %in% c("05104","05201")),codigo_region==filtro_region$codigo_region[3])) %>% dplyr::rename("Código Región" = codigo_region, "Región" = Region))
...
valueBox(value = paste(format(df_daily$active_cum[length(df_daily$active_cum)], big.mark = ","), " (",
                       ...
                       "%)", sep = ""), 
         caption = "Casos Activos", icon = "fas fa-ambulance", 
         color = active_color)
```
...
```{r}
valueBox(value = paste(format(df_daily$recovered_cum[length(df_daily$recovered_cum)], big.mark = ","), " (",
                       round(100 * sum(df_daily$recovered_cum[length(df_daily$recovered_cum)], na.rm = TRUE) / sum(df$Confirmados), 1), 
                       "%)", sep = ""), 
...
mapview(eval(parse(text=paste0("shp_",filtro_region$Region[1]))),
  ...
  alpha = 0.5,
  map.types = c("OpenStreetMap","CartoDB.Positron","CartoDB.DarkMatter","Esri.WorldImagery","OpenTopoMap"),
  layer.name = "Confirmed rate per 100,000 population",
  legend = TRUE,
  at = at_comuna
)
```
...
### Casos Nuevos por Comuna Región Metropolitana
...
Cristian Vásquez con la increíble colaboración de Vania Acevedo y Julián León Vásquez.